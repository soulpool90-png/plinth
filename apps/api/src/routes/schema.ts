...
function quotaExceeded(c: { env: Env }, remaining: number) {
  return json({
    error: "Quota exceeded",
    remaining,
    upgrade_url: upgradeUrl("schema", c.env.PUBLIC_WEB_URL),
  }, 429);
}
...
schemaRoutes.post("/v1/schema/repair", async (c) => {
  ...
  if (!quota.ok) return quotaExceeded(c, quota.remaining);
...
schemaRoutes.post("/v1/schema/saved", async (c) => {
  ...
  if (limits.resources <= 0) {
    return json({ error: "Saved schemas require Pro", upgrade_url: upgradeUrl("schema", c.env.PUBLIC_WEB_URL) }, 402);
  }
  ...
  if ((count?.n ?? 0) >= limits.resources) {
    return json({
      error: "Saved schema limit reached",
      upgrade_url: upgradeUrl("schema", c.env.PUBLIC_WEB_URL),
    }, 402);
  }
